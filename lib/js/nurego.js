/*var nurego_get_jQuery = function () {
			/**
			 * Load AJAX if it is not already loaded
			 *
	        if (!window.jQuery) {
	...
	function Nurego(Api_key) {
		...
		Nurego.prototype.init = function() {
			...
			*/
			...
			this.container.setAttribute('id', 'pricing_table-' + this.api_key);		
			if (this.theme) {
		...
		Nurego.prototype.nr_draw = function(data) {
			...
			// Append table to unique div
			unique_container = document.getElementById('pricing_table-' + this.api_key);
			unique_container.appendChild(table);		    
