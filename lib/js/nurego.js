...
var SIMPLE_3_TIER = 'nr-simple-3-tier',
    FLAT_RATE = 'nr-flat-rate',
    CUSTOM_PRICE = 'nr-custom';

/**
 * Closure
 */
(function () {    
    var SIMPLE_3_TIER_CSS = 'http://js.nurego.com/v1/lib/css/themes.css',
        FLAT_RATE_CSS = '/lib/css/flat_rate.css';
    
    //Default params
    var p = {
        api_key: null,
        element_id: null,
        theme_class: SIMPLE_3_TIER,
        css_url: SIMPLE_3_TIER_CSS,
        select_url: '/?plan_eid=',
        ...
        nurego_url: 'http://localhost:31001/v1/',
        offerings_url: 'http://localhost:31001/v1/offerings?api_key=',
        signup_url: '',
        update_url: '',
    ...
    this.NuregoTheme.setPricingType = function(type) {
      p.theme_class = type;
      if (type === FLAT_RATE) {
        p.css_url = FLAT_RATE_CSS;
      } else if (type === CUSTOM_PRICE) {
        p.css_url = '';
      }
    }
  
    this.NuregoTheme.loadCSS = function() {
      if (p.css_url) {
    ...
    this.NuregoTheme.createPricingTable = function() {
      //Load CSS styles
      this.loadCSS();
      
      if (p.theme_class === SIMPLE_3_TIER) {
        this.createMultiplePrices();
      } else if (p.theme_class === FLAT_RATE) {
        this.createFlatRate();
      }
    ...
    this.NuregoTheme.createMultiplePrices = function() {
      //Create main DOM elements
      container = document.createElement('div');
      if (p.theme_class) {
          container.setAttribute('class', p.theme_class);
      }
    ...
    this.NuregoTheme.createFlatRate = function() {
      container = $(flatRateHTML)[0];
    }
    ...
    function nr_draw(data) {
        ...
        for (i = 0; i < plans.length; i++) {
            ...
            if (p.select_url) {
                ...
                item.setAttribute('data-external-id', plans[i].external_id);
            }
            else {
    ...
    var flatRateHTML =
'<div class="nr-large-16 nr-column nr-price-plans">' + 
  '<div class="nr-large-8 nr-column nr-price-plans-left" id="nr_pp_left">' + 
    '<div class="nr-large-16 nr-column nr-bg-white">' + 
      '<div class="nr-large-16 nr-column nr-price">' + 
        '<div class="nr-price-value">' + 
          '$0.35' + 
        '</div>' + 
        '<div class="nr-price-info">' + 
          ...
          '<ul class="nr-ul">' + 
            '<li>' + 
              '<div class="nr-feature-info">' + 
                '<div class="nr-title">' + 
                  'No fees for your free users' + 
                '</div>' + 
                '<div class="nr-description">' + 
                  'Only get charge for customers who are paying to use your service' + 
                '</div>' + 
              '</div>' + 
            '</li>' + 
            '<li>' + 
  ...
  '<div class="nr-large-8 nr-column nr-price-plans-right" id="nr_pp_right" style="height: 383px;">' + 
    '<div class="nr-large-16 nr-column">' + 
      ...
      '</div>' + 
      '<div class="nr-description">' + 
        'We only charge you for your paid users.' + 
        '<br>' + 
        'Fair and Simple, how it should be.' + 
        '<br>' + 
        '<br>' + 
        'We only charge you for your paid users.' + 
        '<br>' + 
