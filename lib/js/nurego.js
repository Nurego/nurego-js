...
(function () {
    ...
    var p = {
        ...
        element_id: null,
        theme: 'nr-default',
        css_url: 'http://js.nurego.com/v1/lib/css/themes.css',
        select_url: '/?plan_id=',
        ...
        data_url: 'https://api.nurego.com/v1/offerings?api_key=',
    ...
    function nr_draw(data) {
        ...
        $('.nr-signup').on('click', function(e) {
          ...
          var xhr = new easyXDM.Rpc({
              remote: "http://localhost:31001/cors/"
          }, {
              remote: {
          ...
          // create new user in nurego using jsonp

          /*var email = "email+" + (new Date().getTime()) + "@example.com";
          
          if ($('.nr-plan-selected').data("id")) {
            var reg_url = p.nurego_url + 'registrations/jsonp?api_key='
                + p.api_key + "&email=" + encodeURIComponent(email)
            ...
            $.ajax({
                ...
                success:function(json){
                    ...
                    var complete_reg_url = p.nurego_url + 'registrations/' + json.id
                        + '/jsonp_complete?api_key='
                        + p.api_key + '&password=hello';
                    $.ajax({
