(function () {
  ...
  this.Nurego.loadPlans = function loadPlans(html, data, period, switched){
    ...
    // See from the start if there are any monthly plans. If not, set and display yearly plans directly.
    if (!switched){
      var has_monthly = false;
      for (i = 0; i < plans.length; i++) {
        if (plans[i].period == period){
          has_monthly = true;
          break;
        }
      }
      // If plans json has no monthly plans, set as yearly
      if (!has_monthly){
        period = "yearly";
        $('.on-offswitch-label').parent().toggleClass('monthly-checked');
      }
    }

    // Go through each plan
    for (i = 0; i < plans.length; i++) {
        // Skip plans with period other than specified period (month/year)
        ...
        if (parseFloat(plans[i].price) !== 0) {
          ...
          // If we want to have a monthly/yearly switcher displayed
          if (Nurego.p.display_switcher){
            // If display_period flag is set to true, always show monthly/yearly for plans
            if (Nurego.p.display_period){
            ...
            else { // Otherwise, do not show period
              td_price.html(Nurego.p.label_before_price + plan_price + Nurego.p.label_after_price + 
